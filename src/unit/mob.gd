...
func 移动系统():
	if direction != 0 and $"状态机".当前状态 != "奔跑":#角色移动速度从当前速度通过加速度到目标速度
		if direction > 0 and is_on_floor():
			...
			velocity.x = move_toward(velocity.x,-speed,acceleration/3)
	elif direction != 0 and $"状态机".当前状态 == "奔跑":
		if direction > 0 and is_on_floor():
			velocity.x = move_toward(velocity.x,奔跑速度,acceleration)#（角色当前速度，目标速度，加速度）
		elif direction < 0 and is_on_floor():
			velocity.x = move_toward(velocity.x,-奔跑速度,acceleration)
		elif direction > 0 and not is_on_floor():
			velocity.x = move_toward(velocity.x,奔跑速度,acceleration/3)
		elif direction < 0 and not is_on_floor():
			velocity.x = move_toward(velocity.x,-奔跑速度,acceleration/3)
	else:
...
func 玩家跳跃系统():
	...
	if Input.is_action_just_released("A") and is_on_floor():
	...
	if Input.is_action_just_released("A"): #松开跳跃键才能再次跳跃
		跳跃就绪状态 = true
func 玩家攀附系统():
	if Input.is_action_pressed("A") and not is_on_floor() and not 攀附状态 and $"状态机/攀附".能否攀附 and $"状态机".朝向:
		...
		$"状态机".变更状态 = "攀附"
		攀附状态 = true
	elif Input.is_action_pressed("A") and not is_on_floor() and not 攀附状态 and $"状态机/攀附".能否攀附 and not $"状态机".朝向:
