...
func 按键与状态绑定():
	if Input.is_action_pressed("左摇杆向右"):
		...
		朝向 = false
	if Input.is_action_pressed("左摇杆向右") and is_on_floor():
		current_state = State.向右行走
	elif Input.is_action_pressed("左摇杆向左") and is_on_floor():
		current_state = State.向左行走
	elif Input.is_action_pressed("左摇杆按压") and Input.is_action_pressed("左摇杆向左") and is_on_floor():
		current_state = State.向左奔跑
	elif Input.is_action_pressed("左摇杆按压") and Input.is_action_pressed("左摇杆向右") and is_on_floor():
